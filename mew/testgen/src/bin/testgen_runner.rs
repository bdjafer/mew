...
use mew_testgen::{ActualResult, TestConfig, TestExecutor, TestGenerator, TestResult, TestSummary};
use std::fs;
...
fn main() {
    ...
    println!("\n{}", stats);
    std::process::exit(if stats.failed + stats.errors > 0 {
        1
    } else {
        0
    });
    ...
    fn parse() -> Self {
        ...
        while i < args.len() {
            match args[i].as_str() {
                "--level" | "-l" => {
                    i += 1;
                    opts.level = args.get(i).and_then(|s| s.parse().ok());
                }
                "--ontology" | "-o" => {
                    i += 1;
                ...
                "--nodes" | "-n" => {
                    i += 1;
                    opts.nodes = args.get(i).and_then(|s| s.parse().ok()).unwrap_or(5);
                }
                "--queries" | "-q" => {
                    i += 1;
                    opts.queries = args.get(i).and_then(|s| s.parse().ok()).unwrap_or(10);
                }
                "--mutations" | "-m" => {
                    i += 1;
                    opts.mutations = args.get(i).and_then(|s| s.parse().ok()).unwrap_or(5);
                }
                "--output" => {
                    i += 1;
                    opts.output = args.get(i).map(PathBuf::from).unwrap_or(opts.output);
                }
                "--ontologies-dir" => {
                    i += 1;
                    opts.ontologies = args.get(i).map(PathBuf::from).unwrap_or(opts.ontologies);
                }
...
fn print_help() {
    println!(
        r#"Usage: testgen-runner [OPTIONS]
...
fn format_failure(r: &TestResult) -> String {
    match &r.actual {
        ...
        ActualResult::Rows(rows) => {
            ...
            format!(
                "[{}]: got {}, expected {:?}",
                r.test_id, actual_desc, r.expected
            )
        }
        ActualResult::Count(n) => format!(
            "[{}]: got count {}, expected {:?}",
            r.test_id, n, r.expected
        ),
...
fn write_reports(
    ...
    fs::write(
        opts.output.join(format!("{}_report.{}", name, ext)),
        &report,
    )
    .map_err(|e| e.to_string())?;

    // Test cases file
    let cases: String = suite
        .test_cases
        .iter()
        .enumerate()
        .map(|(i, tc)| {
    ...
    fs::write(opts.output.join(format!("{}_tests.txt", name)), cases).map_err(|e| e.to_string())?;
