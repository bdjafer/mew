...
use mew_testgen::{TestConfig, TestGenerator, SchemaAnalyzer, WorldGenerator, QueryGenerator};
use std::fs;
use std::path::Path;
...
fn test_schema_analysis_level1() {
    for (name, source) in find_ontologies(1) {
        ...
        assert!(result.is_ok(), "Failed to analyze {}: {:?}", name, result.err());
...
fn test_world_generation_level1() {
    ...
    for (name, source) in find_ontologies(1) {
        ...
        assert!(result.is_ok(), "Failed to generate world for {}: {:?}", name, result.err());
...
fn test_query_generation_level1() {
    ...
    for (name, source) in find_ontologies(1) {
        ...
        assert!(result.is_ok(), "Failed to generate queries for {}: {:?}", name, result.err());
...
fn test_all_levels_parse() {
    ...
    for level in 1..=5 {
        for (name, source) in find_ontologies(level) {
            let result = SchemaAnalyzer::analyze(&source);
            assert!(result.is_ok(), "Level {} - Failed to analyze {}: {:?}", level, name, result.err());
        }
    }
}
